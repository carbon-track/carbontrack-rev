...
- Text-mode tool result replay should preserve full tool payloads by default; only apply `tool_result_replay_max_bytes` when explicitly configured, because LLM context capacity and audit log capture limits are separate concerns.
