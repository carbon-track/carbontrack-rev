...
-- 2. Idempotency must be keyed by (composite_key, user_id) instead of just key.
...
--    index covers (composite_key, user_id) so different users replaying the same
--    UUID never collide. The legacy idx_idempotency_key unique index is dropped
--    so the same `idempotency_key` UUID can be reused across users.
