...
const MAX_SOLVE_ATTEMPTS = 50000000;
const MAX_SOLVE_MS = 90000;
...
const sha256Bytes = (message) => {
  ...
  const workspace = createSha256Workspace(bytes.length);
  workspace.buffer.set(bytes, 0);
  return new Uint8Array(sha256WorkspaceDigest(workspace, bytes.length));
};

const sha256Hex = (message) => bytesToHex(sha256Bytes(message));

const hexToBytes = (hex) => {
  const normalized = String(hex || '').trim();
  const bytes = new Uint8Array(Math.ceil(normalized.length / 2));
  for (let i = 0; i < bytes.length; i += 1) {
    bytes[i] = parseInt(normalized.slice(i * 2, i * 2 + 2).padEnd(2, '0'), 16) || 0;
  }
  return bytes;
};

const hasLeadingZeroBytes = (bytes, difficulty) => {
  const normalizedDifficulty = Number(difficulty);
  if (!Number.isFinite(normalizedDifficulty) || normalizedDifficulty < 0) {
    return false;
  }

  const fullBytes = Math.floor(normalizedDifficulty / 8);
  ...
  const remainingBits = normalizedDifficulty % 8;
  if (remainingBits === 0) {
    return true;
  }
...
const hasLeadingZeroBits = (hexDigest, difficulty) => hasLeadingZeroBytes(hexToBytes(hexDigest), difficulty);
...
const solveProofOfWork = async (challenge, difficulty, options = {}) => {
  const normalizedDifficulty = Number(difficulty);
  ...
  const maxAttempts = Number.isFinite(options.maxAttempts)
    ...
    : Math.min(MAX_SOLVE_ATTEMPTS, Math.max(2000000, expectedAttempts * 3));
  ...
  while (checked < maxAttempts) {
    ...
    for (; nonce < batchEnd; nonce += 1) {
      if (hasLeadingZeroBytes(hashNonce(nonce), normalizedDifficulty)) {
